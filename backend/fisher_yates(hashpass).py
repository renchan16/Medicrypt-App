...
class Encrypt:
    ...
    def hashArray(self, array, password):
        # Ensure the password is a string
        password_bytes = password.encode('utf-8')
        
        # Combine the array's bytes and the password's bytes
        combined_data = array.tobytes() + password_bytes
        
        # Compute the hash using hashlib
        hash = hashlib.sha512(combined_data).hexdigest()
    ...
    def encryptVideo(self, filepath, vid_destination, key_destination, password, verbose=False):
        ...
        while True:
            ...
            diffuse_pixels, hashed = self.encryptFrame(frame, password, verbose)
